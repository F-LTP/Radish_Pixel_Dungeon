...
public class ScrollOfPsionicBlast extends ExoticScroll {
	...
	public void doRead() {
		...
		for (Mob mob : targets){
			...
			mob.damage(new DamageInfo(Math.round(mob.HT/2f + mob.HP/2f), DamageType.MAGICAL, curUser, this, this));
			if (mob.isAlive()) {
				Buff.prolong(mob, Blindness.class, Blindness.DURATION);
			}
		}
		
		curUser.damage(new DamageInfo(Math.max(0, Math.round(curUser.HT*(0.5f * (float)Math.pow(0.9, targets.size())))), DamageType.TRUE, curUser, this, this));
		if (curUser.isAlive()) {
